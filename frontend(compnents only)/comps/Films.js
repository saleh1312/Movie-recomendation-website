...
function Films(props) {
  ...
  const show = ()=>{
    ...
    let filt_films = state.films.filter((item)=>{
      if (gens.length===0){
        if (sr===true && item.rate===0){
          return false
        }
        if (st===true && item.reco===false){
          return false
        }
        return true;
      }
      ...
      return (
        <div className="card" key={item.name}>
          ...
          <div className="card-info">
              <h3>{item.name.split("_").slice(0, -1).join(" ")}</h3>
              <div className="rating"> 
              <input type="radio" name={item.name} value="5" className={item.name} id={"5"+item.name} onChange={setGender} checked={5===item.rate}/><label htmlFor={"5"+item.name}>☆</label> 
              <input type="radio" name={item.name} value="4" className={item.name} id={"4"+item.name} onChange={setGender} checked={4===item.rate}/><label htmlFor={"4"+item.name}>☆</label> 
              <input type="radio" name={item.name} value="3" className={item.name} id={"3"+item.name} onChange={setGender} checked={3===item.rate}/><label htmlFor={"3"+item.name}>☆</label>  
              <input type="radio" name={item.name} value="2" className={item.name} id={"2"+item.name} onChange={setGender} checked={2===item.rate}/><label htmlFor={"2"+item.name}>☆</label>  
              <input type="radio" name={item.name} value="1" className={item.name} id={"1"+item.name} onChange={setGender} checked={1===item.rate}/><label htmlFor={"1"+item.name}>☆</label> 
              </div>
              
          </div>
          <div className="del" onClick={()=>remove_rate(item.name)}>X</div>
          <div className={item.reco?"recommended":"notrecommended"}>Recommended</div>
